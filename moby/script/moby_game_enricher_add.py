...
class Enricher:
    ...
    def get_bounds(self):
        ...
        cursor = self.cursor 
        result = None
        query = 'SELECT min(id) as minId, max(id) as maxId FROM moby_games ' + \
                'WHERE (themes IS NULL )  AND (sports IS NULL) AND (gameplay IS NULL) ' + \
                'AND (vehicular IS NULL) AND (narrative IS NULL)  ' +\
                'AND (art IS NULL) AND (add_on IS NULL) AND (spec_ed IS NULL) AND (misc IS NULL)'
        print(query)
        try:
    ...
    def update_entry(self,eid,data):
        ...
        if len(data) > 0:
            ...
            if 'Gameplay' in data:
                gameplay = ",".join( data['Gameplay'] )
            if 'Vehicular' in data:
                vahicular = ",".join( data['Vehicular'] )
            if 'Narrative' in data:
                narrative = ",".join( data['Narrative'] )
            if 'Art' in data:
                art = ",".join( data['Art'] )
            if 'Add-on' in data:
                add_on = ",".join( data['Add-on'] )
            if 'Special Edition' in data:
                spec_ed = ",".join( data['Special Edition'] )
            if 'Misc' in data:
                misc = ",".join( data['Misc'] )
                        
            params = (themes,sports,gameplay,vehicular,narrative,art,add_on,spec_ed,misc)
            if g_verbose:
                print(len( params ) )
                try_print(params)
            ...
            if cursor.execute('UPDATE moby_games SET themes = ?,sports = ?,gameplay = ?,vehicular=?,narrative=?,art=?,add_on=?,spec_ed=?,misc=? '+ \
...
#    bounds = (1,20,1)
    print("({minId},{maxId},{count})".format(minId = bounds[0], maxId = bounds[1], count = bounds[1]) )
    for eid in range(bounds[0], bounds[1]+1):
        edata = enricher.get_entry_data(eid)
        approved = edata['approved']
        page_url = edata['url']
        if (approved > 0 ) and ( len(page_url) > 0 ):
